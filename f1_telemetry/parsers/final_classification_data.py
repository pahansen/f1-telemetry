...
class FinalClassificationData:
	...
	_struct = struct.Struct(f"<6BId3B{cs_maxTyreStints}B{cs_maxTyreStints}B{cs_maxTyreStints}B")
	...
	def from_buffer(cls, buffer: bytes, offset: int = 0):
		...
		return cls(
			...
			m_tyreStintsVisual=tuple(unpacked[11+cs_maxTyreStints:11+2*cs_maxTyreStints]),
			m_tyreStintsEndLaps=tuple(unpacked[11+2*cs_maxTyreStints:11+3*cs_maxTyreStints]),
		)
